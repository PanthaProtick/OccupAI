...
export function DashboardPage() {
  ...
  return (
    <>
      ...
      {error && <div className="warning" role="status">Showing the last successful update. {error} <button onClick={() => void refresh()}>Retry</button></div>}
      ...
      >
        <div className="section-heading">
          ...
          <div className="map-filters">
            <label>Building
              <select value={building} onChange={(event) => { setSelectedBuilding(event.target.value); setSelectedFloor(""); }}>
                {buildings.map((name) => <option key={name} value={name}>{name}</option>)}
              </select>
            </label>
            <label>Floor
              <select value={floor} onChange={(event) => setSelectedFloor(event.target.value)}>
                {floors.map((value) => <option key={value} value={value}>{value === 0 ? "Ground floor" : `Floor ${value}`}</option>)}
              </select>
            </label>
          </div>
        </div>
