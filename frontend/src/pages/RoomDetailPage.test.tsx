...
const view = () => render(<MemoryRouter initialEntries={["/rooms/room_canteen"]}><Routes><Route path="rooms/:roomId" element={<RoomDetailPage />} /></Routes></MemoryRouter>);
describe("RoomDetailPage", () => {
  afterEach(() => vi.restoreAllMocks());
  it("loads directly from the route id", async () => { const getRoom = vi.spyOn(api, "getRoom").mockResolvedValue(stale as never); vi.spyOn(api, "getHistory").mockResolvedValue(history as never); view(); expect(await screen.findByRole("heading", { name: "Canteen" })).toBeInTheDocument(); expect(getRoom).toHaveBeenCalledWith("room_canteen", expect.anything()); expect(screen.getByText("Stale")).toBeInTheDocument(); expect(screen.getByRole("link", { name: "Back to dashboard" })).toHaveAttribute("href", "/dashboard?building=University+Building&floor=0"); });
