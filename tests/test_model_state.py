...
class LatestOccupancyStoreTests(unittest.TestCase):
    def test_initial_cameras_are_offline(self) -> None:
        store = LatestOccupancyStore(["cam_001"])
        self.assertEqual(store.snapshot()["cam_001"]["status"], "offline")

    def test_update_preserves_latest_only_state(self) -> None:
        store = LatestOccupancyStore(["cam_001"])
        now = time.time()
        timestamp = datetime.fromtimestamp(now, timezone.utc).isoformat()
        store.update(record("cam_001", timestamp, raw=4, stable=5))
        with patch("model_server.model_state.time.time", return_value=now):
            state = store.camera_snapshot("cam_001")
    ...
    def test_old_state_becomes_stale(self) -> None:
        store = LatestOccupancyStore(["cam_001"], stale_after_seconds=10.0)
        now = time.time()
        timestamp = datetime.fromtimestamp(now, timezone.utc).isoformat()
        store.update(record("cam_001", timestamp))
        with patch("model_server.model_state.time.time", return_value=now + 11.0):
            self.assertEqual(store.camera_snapshot("cam_001")["status"], "stale")

    def test_unknown_camera_is_not_in_store(self) -> None:
        store = LatestOccupancyStore(["cam_001"])
        self.assertIsNone(store.camera_snapshot("cam_999"))
